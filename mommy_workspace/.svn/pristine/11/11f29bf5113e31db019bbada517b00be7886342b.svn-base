	...
	function addList(result) {
		...
		for (var i = 0; i < result.length; i++) {
			...
			if(result[i].ProfilePicture=="" || result[i].ProfilePicture==null ){
				...
				userProfile+="<img src='../images/기본 이미지.png'class='userImgDetail'>";
			}else{
				console.log("픽쳐 널아님")
			...
			userProfile+="<h4 style='margin-bottom: 4px;'><a href='"+contextPath+"/service/LookSitterProfileOk.ser?userNum="+result[i].userNum+"&profile="+result[i].profileNum+"' style='border:none;'>";                                                  
